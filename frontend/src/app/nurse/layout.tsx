...
export default function NurseLayout({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        ...
        { name: 'Antrian Hari Ini', href: '/nurse/queue', icon: <ClipboardList size={16} />, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    ...
    return (
        <div className="flex min-h-screen bg-[#F4F7FE] text-[#1E293B] font-sans">
            ...
            {/* SIDEBAR */}
            <aside className={`fixed lg:sticky lg:top-0 z-50 w-60 h-screen bg-white border-r border-teal-50/50 shadow-sm transition-transform duration-300 flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                {/* Logo */}
                ...
                {/* Badge Role */}
                <div className="mx-3 mt-4 mb-4 p-3 bg-teal-50/40 rounded-xl border border-teal-100/50">
                    <p className="text-[10px] font-black text-teal-900 uppercase leading-none">Nauli Dental Care</p>
                    <p className="text-[9px] text-teal-500 font-bold truncate mt-1 italic opacity-70">{nurseName}</p>
                </div>
            ...
            <main className="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto">
                <header className="h-14 bg-white/80 backdrop-blur-md border-b border-teal-50/50 px-6 flex items-center justify-between sticky top-0 z-40 shrink-0">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-teal-600 uppercase tracking-wider bg-teal-50 px-3 py-1 rounded-full">{nurseName}</span>
                    </div>
                </header>
