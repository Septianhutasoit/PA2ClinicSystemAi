...
def create_staff(
    ...
    current_admin: dict = Depends(security.require_admin) # Hanya Admin yang bisa panggil ini
):
    # Logika sama seperti register, tapi role-nya diambil dari input admin
    new_staff = User(
        email=user_in.email.lower(),
        full_name=user_in.full_name,
        hashed_password=security.get_password_hash(user_in.password),
        role=user_in.role # Admin bebas menentukan 'doctor' atau 'nurse'
    )
    db.add(new_staff)
    db.commit()
